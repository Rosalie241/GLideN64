...
namespace glsl {
	...
	/*---------------ReshadeShader-------------*/
	typedef SpecialShader<ReshadeShader, ReshadeShader> ReshadeShaderBase;

	class ReshadeShader2 : public ReshadeShaderBase
	{
	public:
		ReshadeShader2(const opengl::GLInfo & _glinfo,
			opengl::CachedUseProgram * _useProgram,
			const ShaderPart * _vertexHeader,
			const ShaderPart * _fragmentHeader,
			const ShaderPart * _fragmentEnd)
			: ReshadeShaderBase(_glinfo, _useProgram, _vertexHeader, _fragmentHeader, _fragmentEnd)
		{
			m_useProgram->useProgram(m_program);
			//m_textureSizeLoc = glGetUniformLocation(GLuint(m_program), "uTextureSize");
			m_useProgram->useProgram(graphics::ObjectHandle::null);
	...
	graphics::ShaderProgram * SpecialShadersFactory::createReshadeShader() const
	{
		return new ReshadeShader2(m_glinfo, m_useProgram, m_vertexHeader, m_fragmentHeader, m_fragmentEnd);
	}
